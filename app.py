...
def resolve_direct_url(track_title, track_artists):
    ydl_opts = {
        'format': 'bestaudio/best',
        'noplaylist': True,
        'quiet': True,
    }
    query = f"ytsearch:{', '.join(track_artists)} - {track_title} official audio"
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        ...
        if 'entries' in info and info['entries']:
            ...
            entry = info
        audio_formats = [f for f in entry.get('formats', []) if f.get('vcodec') == 'none' and f.get('acodec') != 'none' and f.get('url')]
        if audio_formats:
...
def api_download(
    ...
    try:
        ...
        def resolve_single(t):
            try:
                direct_url = resolve_direct_url(t['title'], t['artists'])
                t_copy = t.copy()
...
def stream_track(
    ...
    try:
        ...
        direct_url = resolve_direct_url(meta['title'], meta['artists'])
        if not direct_url:
